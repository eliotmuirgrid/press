...
* The binary and its dependencies and configuration should be self-contained. The deployment could also be cryptographically signed so that it cannot be tampered with.
...
* The input feed should be split so that exactly the same HL7 messages can be fed to both implementations.
